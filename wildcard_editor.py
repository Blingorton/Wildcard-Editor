...
# Scintilla drop-in editor
try:
    from scintilla_text import ScintillaText as _SciText
    _USE_SCINTILLA = True
except Exception as _e:
    print(f'[ScintillaText] falling back to tk.Text: {_e}')
    _SciText = None
    _USE_SCINTILLA = False
    ...
    try:
        if p.exists():
            with open(p, "r", encoding="utf-8") as f:
                state = json.load(f)
            changed = False
            for doc in state.get("docs", {}).values():
                # Normalise line endings
                if "content" in doc and doc["content"] and "\r" in doc["content"]:
                    doc["content"] = doc["content"].replace("\r\n", "\n").replace("\r", "\n")
                    changed = True
                # Recompute saved_hash using current hash function (rstrip-normalised)
                # so old stored hashes from before the rstrip change don't cause mismatches.
                if doc.get("content") and not doc.get("modified_unsaved"):
                    doc["saved_hash"] = hashlib.md5(
...
class WildcardEditor:
    ...
    def _post_startup(self):
        ...
        # Re-apply highlights to all open tabs to clear any stale indicator state
        # from before this session (e.g. corrupted bracket highlights on old docs).
        self.root.after(500, self._rehighlight_all_tabs)

    def _rehighlight_all_tabs(self):
        """Clear stale indicators and re-apply highlights on startup."""
        self._clear_and_rehighlight()
    ...
    def _build_main(self):
        ...
        self.line_canvas.bind("<MouseWheel>", self._on_editor_scroll)

        # Separator line
        tk.Frame(self.editor_wrap, bg=COLORS["border2"], width=1).pack(side="left", fill="y")

        # Text editor — created via factory so each tab can have its own widget
        # (self.editor always points to the currently active tab's widget)
        self.editor = None   # set by _make_editor_widget / _switch_tab

        self.ed_scroll = ttk.Scrollbar(self.editor_wrap, orient="vertical")
        self.ed_scroll.pack(side="right", fill="y")

        # Horizontal scrollbar (shown when word-wrap is off)
        self.ed_hscroll = ttk.Scrollbar(self.editor_frame, orient="horizontal")
        if not self.word_wrap.get():
            self.ed_hscroll.pack(fill="x")

        # Mouse side buttons — bound at root level (not per-widget)
        ...
        # self.editor is set to the active tab's widget by _switch_tab


    def _make_editor_widget(self, doc_id):
        """Create and return an editor widget for a tab.
        Uses ScintillaText when available, falls back to tk.Text."""
        ...
        if _USE_SCINTILLA:
            ed = _SciText(self.editor_wrap, **kw)
        else:
            ed = tk.Text(self.editor_wrap, undo=True,
                         autoseparators=False, maxundo=-1,
                         padx=12, pady=8, tabs=("1c",), **kw)
        self._configure_editor_widget(ed)
        return ed

    def _configure_editor_widget(self, ed):
        """Apply all tags, scrollbar wiring, and key bindings to an editor widget."""
        def _on_yscroll(*args):
            self.ed_scroll.set(*args)
            self._sync_scroll()
        ed.config(yscrollcommand=_on_yscroll)
        ed.config(xscrollcommand=self.ed_hscroll.set)

        # Tags
        ed.tag_configure("wildcard",      background=COLORS["wc_hl"], foreground=COLORS["wc_hl_fg"], spacing3=0)
        ed.tag_configure("_pad",          foreground=COLORS["bg0"], background=COLORS["bg0"],
                                          selectforeground=COLORS["bg0"], selectbackground=COLORS["bg0"])
        ed.tag_configure("wc_active",     background=COLORS["accent"], foreground=COLORS["bg0"])
        ed.tag_configure("find_hl",       background="#1e3a1e", foreground=COLORS["accent3"])
        ed.tag_configure("find_cur",      background=COLORS["warn"], foreground=COLORS["bg0"])
        ed.tag_configure("spell_err",     foreground=COLORS["danger"], underline=True)
        ed.tag_configure("warn_tilde",    background=COLORS["warn"], foreground=COLORS["bg0"])
        ed.tag_configure("hl_angle",      background="#1a6b2a", foreground="#b8e8e8")
        ed.tag_configure("bracket_match", font=(self.cfg.get("font_family","Consolas"),
                                                self.cfg.get("font_size",13), "bold"),
                                          foreground="#ffffff")
        ed.tag_configure("sel",           background=COLORS["sel_bg"], foreground=COLORS["text0"])
        _paren_bgs = ["#1d2734","#243445","#2b4156","#324e67","#395b78","#406889","#47759a"]
        for i, bg in enumerate(_paren_bgs):
            ed.tag_configure(f"paren_d{i}", background=bg)
            ed.tag_configure(f"curly_d{i}", background=bg)
        _sqbr_bgs = ["#251914","#342016","#432718","#522e1a","#61351c","#703c1e","#7f4320"]
        for i, bg in enumerate(_sqbr_bgs):
            ed.tag_configure(f"sqbr_d{i}", background=bg)
        ed.tag_raise("sel")

        # Bindings
        ed.bind("<<Modified>>",          self._on_editor_modified)
        ed.bind("<<ScintillaReady>>",     lambda e, w=ed: (
            self.root.after(100, lambda: self._clear_and_rehighlight() if self.editor is w else None),
            self.root.after(30, lambda: self._update_wc_list() if self.editor is w else None),
            self.root.after(50, lambda: self._update_line_numbers() if self.editor is w else None),
        ))
        ed.bind("<<SelectionCleared>>",   lambda e: self.root.after(0, self._apply_wildcard_highlights))
        ed.bind("<<LineCountChanged>>",   lambda e: self._update_line_numbers())
        ed.bind("<KeyRelease>",           self._on_key_release)
        ed.bind("<ButtonRelease-1>",      self._on_editor_click)
        ed.bind("<Button-3>",             self._editor_right_click)
        ed.bind("<Double-Button-1>",      self._on_editor_dbl_click)
        ed.bind("<MouseWheel>",           self._on_editor_scroll)
        ed.bind("<Configure>",            lambda e: self._redraw_line_numbers())
        ed.bind("<Control-s>",            lambda e: (self._save_file(), "break"))
        ed.bind("<Control-S>",            lambda e: (self._save_file(), "break"))
        ed.bind("<Control-n>",            lambda e: (self._new_file(), "break"))
        ed.bind("<Control-N>",            lambda e: (self._new_file(), "break"))
        ed.bind("<Control-w>",            lambda e: (self._close_tab(self.active_tab), "break") if self.active_tab else None)
        ed.bind("<Control-W>",            lambda e: (self._close_tab(self.active_tab), "break") if self.active_tab else None)
        ed.bind("<Control-f>",            lambda e: (self._open_find_with_selection(), "break"))
        ed.bind("<Control-F>",            lambda e: (self._open_find_with_selection(), "break"))
        ed.bind("<Control-h>",            lambda e: (self._open_find_with_selection(), "break"))
        ed.bind("<Control-H>",            lambda e: (self._open_find_with_selection(), "break"))
        ed.bind("<Control-z>",            lambda e: (self._do_undo(), "break"))
        ed.bind("<Control-Z>",            lambda e: (self._do_undo(), "break"))
        ed.bind("<Control-y>",            lambda e: (self._do_redo(), "break"))
        ed.bind("<Control-Y>",            lambda e: (self._do_redo(), "break"))
        ed.bind("<Control-Shift-z>",      lambda e: (self._do_redo(), "break"))
        ed.bind("<Control-Shift-Z>",      lambda e: (self._do_redo(), "break"))
        ed.bind("<KeyRelease-space>",     self._on_snap_key)
        ed.bind("<KeyRelease-Return>",    self._on_snap_key)
        ed.bind("<KeyRelease-Tab>",       self._on_snap_key)
        ed.bind("<KeyRelease-BackSpace>", self._on_snap_key)
        ed.bind("<KeyRelease-Delete>",    self._on_snap_key)
        ed.bind("<<Paste>>",              self._on_snap_boundary)
        ed.bind("<<Cut>>",                self._on_snap_boundary)
        ed.bind("<End>",                  lambda e: self._on_end_key(e))
        ed.bind("<<EndKey>>",             lambda e: self._on_end_key(e))
    ...
    def _switch_tab(self, doc_id, push_nav=True):
        ...
        if doc:
            # ── Swap in the tab's own editor widget ───────────────────────────
            # Each tab has a persistent tk.Text widget that holds its undo stack.
            # We unpack the current widget and pack the new tab's widget in its
            # place — self.editor then points to whichever is currently visible.
            store = self._undo_store.get(doc_id)
            if not store:
                content = doc.get("content", "")
                # If content is empty but a file path exists, read from disk.
                # This handles the case where tree.json lost content after a
                # session where the editor widget failed to render correctly.
                if not content and doc.get("path"):
                    try:
                        with open(doc["path"], "r", encoding="utf-8", errors="replace") as _f:
                            content = _f.read().replace("\r\n", "\n").replace("\r", "\n")
                        doc["content"] = content
                        # Set saved_hash so _check_clean_state can detect undo-back-to-clean
                        if not doc.get("saved_hash"):
                            doc["saved_hash"] = self._content_hash(content)
                    except Exception:
                        pass
                self._seed_undo_store(doc_id, content)
                store = self._undo_store[doc_id]

            # Unpack the old widget (if there is one and it differs)
            if self.editor is not None and self.editor is not store["widget"]:
                self.editor.pack_forget()

            # Pack the new tab's widget and update self.editor
            new_ed = store["widget"]
            if not new_ed.winfo_ismapped():
                new_ed.pack(in_=self.editor_wrap, side="left",
                            fill="both", expand=True, before=self.ed_scroll)
            self.editor = new_ed
            self.editor.config(state="normal")

            # Rewire scrollbars to the new widget
            self.ed_scroll.config(command=self.editor.yview)
            self.ed_hscroll.config(command=self.editor.xview)
        ...
        self._refresh_tree()
        self._update_line_numbers()
        self.root.after(100, self._update_line_numbers)
        self.root.after(300, self._update_line_numbers)
        self._update_status()
        self._update_wc_list()
        # Defer highlighting — HWND not ready synchronously on new tabs
        self.root.after(50, self._clear_and_rehighlight)
    ...
    def _content_hash(self, text):
        ...
        Strips trailing newline to match what _get_real_content() returns."""
    ...
    def _save_file(self, silent=False, doc_id=None):
        ...
        try:
            self._save_file_impl(silent=silent, doc_id=doc_id)
        finally:
            self._save_in_progress = False

    def _save_file_impl(self, silent=False, doc_id=None):
    ...
    def _save_file_as(self, doc_id=None):
        ...
        save_config(self.cfg)
        self._save_file_impl(doc_id=did)
        self._render_tabs()
        self._refresh_tree()
    ...
    def _on_editor_modified(self, event=None):
        ...
        if did and did in self.tree_state["docs"]:
            ...
            if not doc.get("modified_unsaved"):
                ...
                self._refresh_tree()
                self.root.title(f"* {doc['name']} — {APP_NAME}")
        # Debounce heavy UI work — NO snapshot scheduling here.
        # Snapshots are driven by explicit key boundaries (_on_snap_key).
        if self._highlight_job:
            self.root.after_cancel(self._highlight_job)
        self._highlight_job = self.root.after(800, self._deferred_update)
        if self._status_job:
            self.root.after_cancel(self._status_job)
        self._status_job = self.root.after(80, self._deferred_status)

    def _on_snap_key(self, event=None):
        """Insert a native undo separator at word/line boundaries."""
        if self._undo_inhibit:
            return
        try:
    ...
    def _on_snap_boundary(self, event=None):
        """Insert a separator after paste/cut (slight delay so widget commits first)."""
        if self._undo_inhibit:
            return
        self.root.after(10, self._snap_boundary_deferred)

    def _snap_boundary_deferred(self):
        if self._undo_inhibit:
            return
        try:
            self.editor.edit_separator()
    ...
    def _commit_snapshot(self):
        """Legacy shim -- no longer used for scheduling.
        Kept so any remaining call sites are harmless."""
        pass
    ...
    # ── PER-TAB UNDO ─────────────────────────────────────────────────────────
    #
    # Each tab owns its own tk.Text widget stored in _undo_store[doc_id].
    # The native undo stack lives inside that widget and is never touched
    # during tab switches — it is simply swapped in/out of the visible area.
    #
    # _undo_store[doc_id] = {
    #   "widget" : tk.Text   — the tab's persistent editor widget
    #   "cursor" : "ln.col"  — last known cursor position
    # }
    # ─────────────────────────────────────────────────────────────────────────

    def _seed_undo_store(self, doc_id, content):
        """Create and populate a fresh widget for a tab we haven't visited yet."""
        ed = self._make_editor_widget(doc_id)
        ed.config(undo=False)
        ...
        self._undo_store[doc_id] = {
            "widget": ed,
            "cursor": doc.get("cursor", "1.0"),
            "scroll": doc.get("scroll", 0),
        }

    def _save_undo_state(self, doc_id):
        """Called on tab-away: record cursor and scroll position."""
        if not doc_id:
            return
        store = self._undo_store.get(doc_id)
        if store and "widget" in store:
            try:
                store["cursor"] = self.editor.index("insert")
            except Exception:
                pass
            try:
                if hasattr(self.editor, '_sci') and self.editor._hwnd:
                    store["scroll"] = self.editor._sci(2152)
            except Exception:
                pass
        # Also persist to tree_state so it survives restart
        doc = self.tree_state["docs"].get(doc_id)
        if doc and store:
            doc["cursor"] = store.get("cursor", "1.0")
            doc["scroll"] = store.get("scroll", 0)

    def _restore_undo_state(self, doc_id):
        """Called on tab-return: restore cursor and scroll position."""
        store = self._undo_store.get(doc_id)
        if not store:
            return
        try:
            cursor = store.get("cursor", "1.0")
            self.editor.mark_set("insert", cursor)
        except Exception:
            pass
        # Restore scroll — defer slightly so HWND is fully laid out
        scroll = store.get("scroll", 0)
        if scroll and hasattr(self.editor, '_sci') and self.editor._hwnd:
            try:
                self.editor._sci(2613, scroll)
            except Exception:
                pass
        elif scroll:
            self.root.after(50, lambda: self._restore_scroll(doc_id))

    def _restore_scroll(self, doc_id):
        """Deferred scroll restore for tabs whose HWND wasn't ready immediately."""
        if doc_id != self.active_tab:
            return
        store = self._undo_store.get(doc_id)
        if not store:
            return
    ...
    def _cleanup_undo_store(self, doc_id):
        store = self._undo_store.pop(doc_id, None)
        if store and "widget" in store:
            try:
    ...
    def _push_undo_snapshot(self, doc_id=None):
        """Insert a native undo separator.  All old call-sites that used to push
        a custom snapshot now just drop a separator — same logical effect."""
        if self._undo_inhibit:
            return
        # Only meaningful for the active tab (native stack is per-widget)
        if doc_id and doc_id != self.active_tab:
            return
        try:
            self.editor.edit_separator()
        except Exception:
    ...
    def _flush_snap_timer(self):
        """Legacy shim — no timer exists any more.  Drop a separator so any
        unsaved typing before a tab-away or undo is captured as its own step."""
        if self._undo_inhibit:
            return
        try:
            self.editor.edit_separator()
    ...
    def _get_or_create_undo_widget(self, doc_id):
        """Compat shim."""
        return self._undo_store.setdefault(doc_id, {"content": "", "cursor": "1.0"})

    def _after_edit_op(self):
        """Shared cleanup after edit_undo or edit_redo."""
        # Reapply scroll padding (undo may have wiped it since it was added
        # with undo=False — the padding itself won't be undone but the cursor
        # may need clamping).
        self._undo_inhibit = True
        try:
            self._apply_scroll_padding()
    ...
    def _apply_snapshot(self, content, cursor):
        """Load content into the editor.  Used by programmatic operations
        (replace-all, LoRA adjust) that want a clean undo boundary around them."""
        self._undo_inhibit = True
        try:
            self._remove_scroll_padding()
            self.editor.config(undo=False)
    ...
    def _on_editor_scroll(self, event):
        ...
        if event.state & 0x4:
            ...
            if new_size != self.cfg["font_size"]:
                ...
                for store in self._undo_store.values():
                    w = store.get("widget")
                    if w:
                        try:
                            w.config(font=(fam, new_size))
                            w.tag_configure("bracket_match", font=(fam, new_size, "bold"), foreground="#ffffff")
                        except Exception:
                            pass
                self._apply_scroll_padding()
                self._redraw_line_numbers()
    ...
    def _line_canvas_click(self, event):
        ...
        try:
            ...
            if _USE_SCINTILLA and hasattr(self.editor, '_sci'):
                sci = self.editor._sci
                ln0 = best_ln - 1          # 0-based
                p1  = sci(2167, ln0)       # SCI_POSITIONFROMLINE (anchor/start)
                p2  = sci(2136, ln0)       # SCI_GETLINEENDPOSITION (caret/end)
                self.editor.focus_set()    # give focus to Scintilla HWND first
                sci(2160, p1, p2)          # SCI_SETSEL(anchor, caret) — atomic
            else:
                self.editor.tag_remove("sel", "1.0", "end")
                self.editor.tag_add("sel", f"{best_ln}.0", f"{best_ln}.end")
                self.editor.mark_set("insert", f"{best_ln}.0")
                self.editor.see(f"{best_ln}.0")
    ...
    def _apply_wildcard_highlights(self):
        ...
        # Scintilla handles this efficiently; the low limit was for tk.Text.
        bracket_limit = 500_000 if (_USE_SCINTILLA and hasattr(self.editor, '_sci')) else 50_000
        if len(content) < bracket_limit:
        ...
        # Re-stamp find highlights on top — wildcard/bracket repaint would
        # otherwise stomp over them since they repaint all indicators.
        self._restamp_find_highlights()

    def _clear_and_rehighlight(self):
        """Wipe indicator slots then re-apply highlights cleanly."""
        if self.editor is None:
            return
        if _USE_SCINTILLA and hasattr(self.editor, '_sci'):
            sci = self.editor._sci
            if not self.editor._hwnd:
                return
            byte_len = sci(2006)   # SCI_GETLENGTH
            if byte_len > 0:
                # Only clear the slots we actually repaint: brackets(9-29),
                # wildcard(30), angle(31), find(32-33), spell(34), wc_active(35)
                # paren_span(36) and brace_bold(37) are cleared by update_brace_highlight
                for slot in range(8, 36):
                    sci(2500, slot)
                    sci(2505, 0, byte_len)
            self.editor._find_hl_ranges = []
            self.editor._tag_ranges.pop("spell_err", None)
            for _m in range(1, 6):  # clear all 5 depth markers
                sci(2045, _m)
        self._apply_wildcard_highlights()
        if self.spell_enabled:
            self.root.after(50, self._run_spell_check)

    def _restamp_find_highlights(self):
        """Re-apply find_hl / find_cur indicators after a wildcard/bracket repaint.
        Wildcard highlights repaint all indicators, stomping over find highlights.
        This re-stamps them so find always visually wins."""
        if not hasattr(self, "find_matches") or not self.find_matches:
            return
        for s, e in self.find_matches:
            self.editor.tag_add("find_hl", s, e)
        if 0 <= getattr(self, "find_current", -1) < len(self.find_matches):
    ...
    def _apply_bracket_highlights(self):
        ...
        content = self._get_real_content()
        ed = self.editor
        # Scintilla handles bracket coloring via indicators in the wrapper
        if _USE_SCINTILLA and hasattr(ed, '_highlight_brackets'):
            ed._highlight_brackets(content)
            return
    ...
    def _apply_depth_tags(self, content, open_ch, close_ch, tag_prefix):
        ...
        Matched pairs highlight at their nesting depth.
        Unmatched open  brackets highlight forward to end-of-content.
        Unmatched close brackets highlight backward to start-of-content (symmetric).
        ...
        # ── First: find which positions are unmatched opens/closes ───────────
        # Stack-based matching pass to identify unmatched bracket positions.
        open_stack = []       # indices of unmatched open brackets so far
        unmatched_closes = [] # indices of unmatched close brackets
        for i, ch in enumerate(content):
            if ch == open_ch:
                open_stack.append(i)
            elif ch == close_ch:
                if open_stack:
                    open_stack.pop()   # matched — remove from open stack
                else:
                    unmatched_closes.append(i)
        unmatched_opens = set(open_stack)   # whatever remains is unmatched
        unmatched_closes_set = set(unmatched_closes)

        # ── Forward pass: depth from matched pairs + unmatched opens ─────────
        # depth_fwd[i] = nesting depth AT position i.
        # Open bracket raises depth before recording; close lowers after.
        # Unmatched closes are completely ignored here.
        depth_fwd = [0] * n
        d = 0
        for i, ch in enumerate(content):
            if ch == open_ch:
        ...
        # ── Backward pass: depth from unmatched closes only ──────────────────
        # Scan right-to-left. Only UNMATCHED close brackets raise the
        # backward depth; matched closes and all opens are ignored.
        # This ensures matched pairs are never given spurious backward depth.
        depth_bwd = [0] * n
        d = 0
        for i in range(n - 1, -1, -1):
            if i in unmatched_closes_set:
                d += 1
            depth_bwd[i] = d

        # ── Combine ──────────────────────────────────────────────────────────
        depth_at = [max(depth_fwd[i], depth_bwd[i]) for i in range(n)]
    ...
    def _update_bracket_highlights(self):
        ...
        ed = self.editor
        # In Scintilla mode use native SCI_BRACEHIGHLIGHT instead of tag_add
        if _USE_SCINTILLA and hasattr(ed, 'update_brace_highlight') and ed._hwnd:
            try:
                pos = ed._sci(2008)  # SCI_GETCURRENTPOS
                ed.update_brace_highlight(pos)
            except Exception:
                pass
            return
    ...
    def _toggle_word_wrap(self):
        ...
        # Update ALL tab widgets so wrap is consistent when switching tabs
        for store in self._undo_store.values():
            w = store.get("widget")
            if w:
                try:
    ...
    def _toggle_find(self):
        if self.find_frame.winfo_ismapped():
            ...
            self.root.after(50, self.find_entry.focus_force)
    ...
    def _replace_current(self):
        ...
        # Bracket the replacement with separators so it's one undoable step
        self.editor.edit_separator()
    ...
    def _replace_all(self):
        ...
        if count:
            self._apply_snapshot(new_content, self.editor.index("insert"))
            did = self.active_tab
            if did and did in self.tree_state["docs"]:
                self.tree_state["docs"][did]["content"] = new_content
                self.tree_state["docs"][did]["modified_unsaved"] = True
        ...
        def do_replace_all():
            ...
            for did, doc in self.tree_state["docs"].items():
                ...
                if did == self.active_tab:
                    ...
                    w = store.get("widget") if store else None
                    if w:
                        try:
                            content = w.get("1.0", "end-1c")
                            # Strip scroll padding if present
                            pad_start = w.tag_ranges("_pad")
                            if pad_start:
                                content = w.get("1.0", str(pad_start[0])).rstrip("\n")
                        except Exception:
                            content = doc.get("content", "")
                    else:
                        content = doc.get("content", "")
                ...
                if did == self.active_tab:
                    ...
                    # Update the tab's widget text (preserves undo stack context)
                    store = self._undo_store.get(did)
                    w = store.get("widget") if store else None
                    if w:
                        try:
        ...
        def do_adjust(scope):
            ...
            for did, doc in docs_to_process.items():
                ...
                if did == self.active_tab:
                    ...
                    # Update the inactive tab's widget so it's current on switch
                    store = self._undo_store.get(did)
                    w = store.get("widget") if store else None
                    if w:
                        try:
    ...
    def _show_settings(self):
        ...
        def save_s():
            ...
            for store in self._undo_store.values():
                ...
                if w:
                    try:
                        w.config(font=(fam, sz))
                        w.tag_configure("bracket_match", font=(fam, sz, "bold"), foreground="#ffffff")
                    except Exception:
                        pass
            self._apply_scroll_padding()
            self._redraw_line_numbers()
    ...
    def _bind_keys(self):
        ...
        r.bind("<Alt-w>",                 lambda e: self._toggle_word_wrap())
        ...
        r.bind("<Control-z>",             lambda e: (self._do_undo(), "break"))
        r.bind("<Control-y>",             lambda e: (self._do_redo(), "break"))
        r.bind("<Control-Y>",             lambda e: (self._do_redo(), "break"))
        r.bind("<Control-Shift-z>",       lambda e: (self._do_redo(), "break"))
        r.bind("<Control-Shift-Z>",       lambda e: (self._do_redo(), "break"))
